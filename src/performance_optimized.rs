// Optimized performance metrics and FPS tracking with ring buffer
use std::time::{Duration, Instant};
use crate::ring_buffer::DurationRingBuffer;

/// Accurate FPS counter using ring buffer for zero-allocation tracking
pub struct FpsTracker {
    // Frame timing with ring buffer (60 samples)
    frame_times: DurationRingBuffer<60>,
...
impl FpsTracker {
    pub fn new() -> Self {
        let now = Instant::now();
        Self {
            frame_times: DurationRingBuffer::new(),
    ...
    pub fn frame_rendered(&mut self, _frame_time: Duration) {
        ...
        if self.total_frames > 1 {
            // Store time between frames using ring buffer
    ...
    fn update_statistics(&mut self) {
        ...
        if let Some(avg_frame_time) = self.frame_times.average() {
        ...
        if let Some(recent_avg) = self.frame_times.average_last_n(10) {
...
/// Performance metrics beyond just FPS with optimized storage
pub struct PerformanceMetrics {
    pub fps_tracker: FpsTracker,
    
    // Timing breakdown with ring buffers
    render_times: DurationRingBuffer<30>,
    flush_times: DurationRingBuffer<30>,
...
impl PerformanceMetrics {
    pub fn new() -> Self {
        Self {
            ...
            render_times: DurationRingBuffer::new(),
            flush_times: DurationRingBuffer::new(),
    ...
    pub fn record_flush_time(&mut self, duration: Duration) {
        ...
        self.flush_times.push(duration);
    }
    ...
    pub fn average_flush_time(&self) -> Option<Duration> {
        self.flush_times.average()
    }
}

// Re-export the original module's functionality for compatibility
pub use crate::performance::{FpsStats as OriginalFpsStats};
...
mod tests {
    ...
    fn test_fps_tracker() {
        ...
        // Simulate 60 FPS (16.67ms between frames)
        for _ in 0..10 {
            tracker.frame_rendered(Duration::from_micros(16667));
