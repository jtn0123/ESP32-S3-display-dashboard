...
impl StableServerConfig {
    ...
    pub fn create() -> Configuration {
        ...
        Configuration {
            // Rust needs more stack than C - but 10KB might be too much
            stack_size: 8192,  // 8KB is standard for ESP-IDF HTTP server
            
            // Support all routes with headroom - increased to 50 for SSE endpoints
            max_uri_handlers: 50,
            
            // Most other fields may not be available in esp-idf-svc's Configuration struct
            // Using defaults for everything else to avoid ESP_ERR_INVALID_ARG
            ..Default::default()
        }
    }
    ...
    pub fn log_completion(&self, path: &str, status: u16) {
        unsafe {
            ...
            if let Some(ref id) = self.request_id {
                log::info!(
                    "[{}] {} {} - {}ms heap_delta:{} largest:{} stack_used:{}",
                    id, path, status, duration_ms, heap_delta, end_heap_largest, stack_used
                );
            } else {
                log::info!(
                    "{} {} - {}ms heap_delta:{} largest:{} stack_used:{}",
                    path, status, duration_ms, heap_delta, end_heap_largest, stack_used
                );
            }
